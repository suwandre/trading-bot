/**
 * Grid Trading Strategy Backtest Example
 * 
 * This strategy is designed for:
 * - Sideways/ranging markets
 * - High trade frequency with small profits
 * - Low drawdown
 */
...
const gridStrategyConfig: StrategyConfig = {
  id: 'grid-btc-sideways',
  name: 'Grid Trading BTC Sideways',
  type: 'grid',
  symbol: 'BTC/USDT',
  timeframe: '15m',
  mode: 'backtest',
  parameters: {
    ...
    upperPriceOffset: 7.5,   // 7.5% above current price
    lowerPriceOffset: 7.5,    // 7.5% below current price
...
async function runGridBacktest() {
  ...
  console.log('GRID TRADING BACKTEST');
  console.log('='.repeat(80));
  console.log('');
  console.log('Strategy Overview:');
  console.log('  - Buys at lower grid levels, sells at upper grid levels');
  console.log('  - Profits from price oscillating within a range');
  console.log('  - High trade frequency, small profits per trade');
  ...
  try {
    ...
    console.log(`  Grid Levels: ${gridStrategyConfig.parameters.gridLevels}`);
    console.log(`  Grid Spacing: ${gridStrategyConfig.parameters.gridSpacingPercent}%`);
    console.log('');
    ...
    console.log('='.repeat(80));
    console.log(`                      Current DCA    Grid Trading`);
...
runGridBacktest().catch((error) => {
  console.error('Fatal error:', error);
  process.exit(1);
});

export { runGridBacktest };
